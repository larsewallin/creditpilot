-- Removes the LIMIT 25 from fn_rank_portfolio_risk(), found during the CIA
-- list-completeness audit. The function's ORDER BY already puts is_high_risk
-- customers first, so the limit never truncated the locked V1 high-risk rule
-- itself (confirmed: same 7 customers before and after) -- but it silently
-- capped every other customer-level question (credit rating, scenario,
-- risk_tags, payment_health) to ~25 of 59 customers for portfolio-wide
-- questions, and would have silently truncated the locked V1 rule itself if
-- the genuine high-risk set ever grew past 25. Removing entirely rather than
-- raising it, since there's no real reason for any cap at this portfolio size.
