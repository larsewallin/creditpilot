-- B5: portfolio risk ranking function.
-- Implements the locked V1 rule (CreditPilot_Risk_Ranking_Priority_V1.md):
--   Membership: current_exposure > 0 AND (score<30 OR bankruptcy[4 triggers])
--   Ranking within set: exposure DESC, 90d severity_score sum DESC, latest event DESC NULLS LAST
-- Returns ALL customers (high-risk first, then the rest by exposure) so portfolio
-- questions keep full breadth while risk-ranked customers lead the context.

CREATE OR REPLACE FUNCTION fn_rank_portfolio_risk()
RETURNS TABLE (
  id uuid,
  company_name text,
  ticker text,
...
LANGUAGE sql STABLE AS $$
  ...
  SELECT
    c.id, c.company_name, c.ticker, c.company_type,
  ...
  ORDER BY
    ...
    ) DESC,                                              -- high-risk set first
    c.current_exposure DESC,                             -- then exposure
    COALESCE(sev.recent_severity_sum,0) DESC,            -- then 90d severity sum
    COALESCE(sev.latest_event_date, ea.latest_event_date_all) DESC NULLS LAST,  -- then recency
    c.company_name                                       -- stable final tiebreak
  LIMIT 25;
$$;
