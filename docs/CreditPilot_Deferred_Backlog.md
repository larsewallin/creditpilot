...
- **INDUSTRY_DOWNTURN — PINNED** (Industry Risk Monitor build). Three open questions resolved: (1) `indicator` is now a controlled enum — `production_output | producer_prices | employment | commodity_price | capacity_utilization` — not free text, with an optional `source_series` field (e.g. "FRED:IPMAN") for traceability back to the originating government series. (2) `period_days` is pinned at 365 (year-over-year), matching how FRED/BLS series are actually reported — not a rolling daily window like Payment Behaviour's 30-day windows. (3) `change_percent`'s sign is normalized to the sector's exposure direction (producer vs consumer of the underlying indicator) before it's written to the payload — negative always means deterioration for that sector's actual exposure, never a raw unsigned market move. See the sector exposure-direction table in the Industry Risk Monitor agent for the producer/consumer mapping per sector.
...
**Real cost data collected and limit raised from 5 to 10/day (2026-09-13, commit 78cc9f3).** The same day the 5/day limit shipped, a real second device on the founder's own home network got blocked -- a genuine consequence of IP-based limiting: every device on one household/office network shares a single public IP, so one person's own testing can exhaust the shared daily allowance for anyone else on that network. This is an inherent tradeoff of IP-based limiting, not a bug -- there's no way to distinguish "one visitor with multiple devices" from "one script abusing the endpoint" using IP alone, and alternatives (cookies, fingerprinting) have their own easy bypasses.
...
## sec-monitor-agent demo state drift -- found and fixed (2026-09-16, commit a9bd77c)

Found via a live screenshot: the SEC Filings page showed Triumph Group Inc and Heliogen Inc as "Alert Active," but the Credit Events page's SEC tab showed zero SEC-sourced events at all. Initial hypothesis was a frontend filtering bug -- checked the live database directly instead of trusting the premise, and found the frontend was correctly displaying empty (zero sec_monitor_agent credit_events genuinely existed at that moment).

Root cause, fully traced: MAX_CUSTOMERS_PER_RUN=10 was applied uniformly to both the live path (a real constraint -- sequential EDGAR HTTP calls per customer, genuine execution-time and third-party-courtesy-rate-limit concern) and the demo path (a single indexed Postgres lookup against a 2-row seed table, zero external calls, no real constraint at all). Combined with an unordered query and a self-reset that unconditionally wipes ALL of the agent's prior credit_events/sec_filings output on every run regardless of which subset gets reprocessed, any demo customer excluded from that run's arbitrary 10-of-47 selection had its genuine evidence deleted with nothing to regenerate it. This wasn't a one-off -- confirmed via agent_runs history that two subsequent runs (both triggered incidentally, likely by page loads / initDemo) had already silently wiped Triumph and Heliogen's real alert evidence this way, while sec_monitoring's alert_triggered flag was never cleared, since it's a one-way ratchet with no corresponding "clear" path.
